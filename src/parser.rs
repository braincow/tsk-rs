use nom::{bytes::complete::take_while1, IResult, character::{is_space, is_newline, complete::char}, sequence::{preceded, separated_pair}, branch::alt, combinator::map};
...
fn directive(input: &str) -> IResult<&str, Expression> {
    ...
    map(project, Expression::Project),
    map(metadata, |(key, value) | Expression::Metadata {key, value}),
    ))(input)
}
...
pub fn parse(input: &str) -> IResult<&str, Vec<Expression>> {
    ...
    while !current_input.is_empty() {
            ...
            match directive(&current_input[current_index..]) {
                Ok((remaining, parsed)) => {
                    ...
                    if !leading_text.is_empty() {
                        output.push(Expression::Description(leading_text));
                    }
                    output.push(parsed);
        ...
        if !found_directive {
            ...
            output.push(Expression::Description(current_input.trim()));
            break;
        }
    }
...
mod tests {
    ...
    fn parse_full_testcase() {
        ...
        let (leftover, mut meta) = parse(input).unwrap();

        assert_eq!(leftover, "");
        // assert the expressions from Vec
        assert_eq!(meta.pop().unwrap(), Expression::Description("additional text at the end"));
        assert_eq!(meta.pop().unwrap(), Expression::Metadata { key: "fuu", value: "bar" });
        assert_eq!(meta.pop().unwrap(), Expression::Metadata { key: "x-meta", value: "data" });
        assert_eq!(meta.pop().unwrap(), Expression::Hashtag("a-second-tag"));
        assert_eq!(meta.pop().unwrap(), Expression::Hashtag("taghere"));
        assert_eq!(meta.pop().unwrap(), Expression::Project("project-here"));
        assert_eq!(meta.pop().unwrap(), Expression::Description("some task description here"));
    }
    ...
    fn parse_full_testcase_no_expressions() {
        ...
        assert_eq!(meta.pop().unwrap(), Expression::Description(input));
